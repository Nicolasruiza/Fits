(()=>{
  ...
  function ensureStyle(){if(document.getElementById('fits-smart-filter-style'))return;const style=document.createElement('style');style.id='fits-smart-filter-style';style.textContent='.card.smart-hidden{display:none!important}';document.head.appendChild(style)}
  function enhance(){
    observer?.disconnect();ensureStyle();
    ...
    cards.forEach(card=>{
      ...
      if(!best){delete card.dataset.smartLook;delete card.dataset.smartScore;return}
      ...
      const visual=card.querySelector('.visual');if(visual){let pill=visual.querySelector('.readiness-pill');if(!pill){pill=document.createElement('span');visual.appendChild(pill)}pill.className=`readiness-pill ${ready?'ready':'need'}`;const t=hasWardrobe?(ready?'READY':`${best.m.length} TO GO`):'SET WARDROBE';if(pill.textContent!==t)pill.textContent=t}
    ...
    const sorted=ranked.sort((a,b)=>b.score-a.score).map(x=>x.card);sorted.forEach(c=>grid.appendChild(c));
    const empty=grid.querySelector('.empty');if(empty){if(grid.lastElementChild!==empty)grid.appendChild(empty);const visible=ranked.filter(x=>!x.card.classList.contains('hidden')).length;empty.style.display=visible?'none':'block';if(!visible)empty.textContent='No looks match the current weather and filters.'}
    ...
    const visibleRanked=ranked.filter(x=>!x.card.classList.contains('hidden'));
