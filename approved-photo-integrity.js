...
{id:'look-46',family:'family-blazer-white-beige',name:'Grey blazer + white shirt + cream trousers + brown loafers',image:'assets/look-46-grey-blazer-cream.webp?v=3'}
];
const look=id=>(D.looks||[]).find(x=>x.id===id);
approved.forEach(a=>{const l=look(a.id);if(!l)return;if(a.image)l.image=a.image;l.inspiration=false;l.family=a.family;if(a.id==='look-46'&&!l.pieces.includes('white-shirt'))l.pieces.splice(1,0,'white-shirt');let f=(D.families||[]).find(x=>x.id===a.family);if(!f){f={id:a.family,name:a.name,hero:a.id,looks:[a.id]};D.families.push(f)}else{f.looks=(f.looks||[]).filter(id=>!!look(id));if(!f.looks.includes(a.id))f.looks.push(a.id);if(!look(f.hero))f.hero=a.id;}});
